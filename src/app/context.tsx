...
interface AppContextValue {
  ...
  uploadMaterialFile: (file: File) => Promise<UploadedMaterialFile>;
  ...
  saveProgress: (lessonId: string, payload: Partial<LessonProgress>, playbackToken: string) => void;
}
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const commitOptimisticState = (applyLocal: (current: AppState) => AppState, request: Promise<AppState>) => {
    latestStateSequenceRef.current += 1;
    const sequence = latestStateSequenceRef.current;
    setState(applyLocal);
    void request
      ...
      .catch((error) => {
        logApiError(error);
        if (sequence === latestStateSequenceRef.current) {
          void applyLatestState(fetchServerState(), setState).catch(logApiError);
        }
      });
  };

  const saveEntity = <K extends EntityKey>(key: K, item: EntityValue<K>) => {
    commitOptimisticState((current) => replaceEntity(current, key, item), saveEntityRequest(key, item));
  };

  const archiveEntity = <K extends EntityKey>(key: K, id: string) => {
    commitOptimisticState((current) => archiveEntityLocal(current, key, id), archiveEntityRequest(key, id));
  };
  ...
  const saveProgress = (lessonId: string, payload: Partial<LessonProgress>, playbackToken: string) => {
    void applyLatestState(saveProgressRequest(lessonId, payload, playbackToken), setState).catch(logApiError);
  };
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        uploadMaterialFile: uploadMaterialFileRequest,
