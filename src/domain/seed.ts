...
const workshopFoldersBlueprint = [
  ...
  {
    title: "Workbook воркшопа",
    description: "Один комплект таблиц и шаблонов, чтобы собрать маршрут за 5 уроков.",
    kind: "folder" as const,
    coverStyle: "notebook",
    ...
    items: [
      ...
      { title: "План на 7 дней", type: "template" as const, body: "Чеклист публикаций, Telegram CTA и первых метрик на неделю." },
    ],
  },
];
...
const workshopModule: Module = ensureCover({
  id: workshopModuleId,
  trainingId: workshopTrainingId,
  title: "Маршрут воркшопа",
  description: "Пять уроков, которые собирают первую рабочую связку без магии и серых схем.",
  order: 1,
  status: "published",
  accessPolicy: policy(["workshop"], "Открыт участникам воркшопа"),
  coverStyle: "brief",
