    ...
    <section className="board-section">
      ...
      <div className="folder-grid">
        {folders.map((folder) => {
          ...
          if (!folderAccess.allowed) {
            return (
              <article className="chalk-card locked-card" key={folder.id}>
                {content}
              </article>
            );
          }
          if (folder.kind === "external" && folder.externalUrl) {
            return (
              <a className="chalk-card folder-card" href={folder.externalUrl} target="_blank" rel="noreferrer" key={folder.id}>
                {content}
              </a>
            );
          }
          return (
            <Link className="chalk-card folder-card" to={`/trainings/${training.id}/folders/${folder.id}`} key={folder.id}>
              {content}
            </Link>
          );
        })}
      ...
      <div className="module-grid">
        {modules.map((module) => {
          ...
          return (
            <article className={moduleAccess.allowed ? "chalk-card module-card" : "chalk-card module-card locked-card"} key={module.id}>
